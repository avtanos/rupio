...
interface ReadyPoiOrderFormProps {
  ...
  onSave: (order: ReadyPoiOrder) => void;
  onCancel: () => void;
  productTypes: ReadyPoiProductType[];
  diagnoses: ReadyPoiDiagnosis[];
  materialUnits: string[];
  defaultMaterials: ReadyPoiMaterial[];
}

const ReadyPoiOrderForm: React.FC<ReadyPoiOrderFormProps> = ({
  order,
  personalFile,
  personalFiles = [],
  onSave,
  ...
  const [formData, setFormData] = useState<ReadyPoiOrderFormData>({
    productType: 'Кресло-коляска',
    diagnosis: 'G80 - Детский церебральный паралич',
    orderType: 'Первичный',
  ...
  const handleInputChange = (field: keyof ReadyPoiOrderFormData, value: string | number | boolean | ReadyPoiMaterial[] | ReadyPoiFitting[]) => {
  ...
  const handleAddFitting = () => {
    ...
    const newFitting: ReadyPoiFitting = {
      id: `fit_${Date.now()}`,
      fittingNumber,
  ...
  const generateOrderNumber = () => {
    ...
    return `ПОИ-${currentYear}-${randomNumber.toString().padStart(3, '0')}`;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      const orderData: ReadyPoiOrder = {
        id: order?.id || `POI${Date.now()}`,
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">
          {order ? 'Редактирование заказа на готовые ПОИ' : 'Заказ на готовые ПОИ'}
        </h2>
        <button
          onClick={onCancel}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('productType', e.target.value as ReadyPoiProductType)}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Диагноз (МКБ-10/МКФ) <span className="text-red-500">*</span>
              </label>
              <select
                value={formData.diagnosis}
                onChange={(e) => handleInputChange('diagnosis', e.target.value as ReadyPoiDiagnosis)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('orderType', e.target.value as ReadyPoiOrderType)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('status', e.target.value as ReadyPoiOrderStatus)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('serviceType', e.target.value as ReadyPoiServiceType)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
...
export default ReadyPoiOrderForm;

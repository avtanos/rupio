...
import { Plus, Search, Filter, Edit, Eye, Trash2, Printer, Download, CheckCircle, Clock, AlertCircle, Package, User, Calendar, DollarSign, FileText, Footprints, X } from 'lucide-react';
import { FootwearInvoice, InvoiceFilters, InvoiceStats, InvoiceStatus, InvoicePriority } from '../types/invoices';
import { PersonalFile } from '../types/personalFile';

interface FootwearInvoicesPageProps {
  invoices: FootwearInvoice[];
  personalFiles: PersonalFile[];
  onNewInvoice: (invoice: Omit<FootwearInvoice, 'id' | 'createdAt' | 'updatedAt'>) => void;
  onUpdateInvoice: (id: string, updates: Partial<FootwearInvoice>) => void;
  onDeleteInvoice: (id: string) => void;
  onPrintInvoice: (id: string) => void;
}

const FootwearInvoicesPage: React.FC<FootwearInvoicesPageProps> = ({
  invoices,
  personalFiles,
  onNewInvoice,
  onUpdateInvoice,
  ...
  const [selectedInvoice, setSelectedInvoice] = useState<FootwearInvoice | null>(null);
  const [showInvoiceForm, setShowInvoiceForm] = useState(false);
  const [editingInvoice, setEditingInvoice] = useState<FootwearInvoice | null>(null);
  ...
  const handleNewInvoice = () => {
    const newInvoice: FootwearInvoice = {
      id: '',
      invoiceNumber: '',
  ...
  const handleEditInvoice = (invoice: FootwearInvoice) => {
    setEditingInvoice(invoice);
    setShowInvoiceForm(true);
  };

  const handleViewInvoice = (invoice: FootwearInvoice) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Накладные на обувь</h1>
          <p className="text-gray-600">Управление накладными на изготовление ортопедической обуви</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
              ...
              {filteredInvoices.map((invoice) => {
                ...
                return (
                  <tr key={invoice.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">{invoice.model}</div>
                      <div className="text-sm text-gray-500">Цвет: {invoice.color}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">{invoice.size}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
        ...
        {filteredInvoices.length === 0 && (
          <div className="text-center py-12">
            <Footprints className="mx-auto h-12 w-12 text-gray-400" />
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold mb-3">Характеристики обуви</h3>
                    <div className="space-y-2">
                      <div><strong>Модель:</strong> {selectedInvoice.model}</div>
                      <div><strong>Цвет:</strong> {selectedInvoice.color}</div>
                      <div><strong>Материал:</strong> {selectedInvoice.material}</div>
                      <div><strong>Размер:</strong> {selectedInvoice.size}</div>
                      <div><strong>Высота каблука:</strong> {selectedInvoice.heelHeight} см</div>
                      <div><strong>Материал каблука:</strong> {selectedInvoice.heelMaterial}</div>
                      <div><strong>Слепок:</strong> {selectedInvoice.needsCast ? 'Да' : 'Нет'}</div>
                    </div>
                  </div>

                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold mb-3">Технические операции</h3>
                    <div className="space-y-2">
                      {selectedInvoice.technicalOperations.map((operation, index) => (
                        <div key={operation.id} className="border border-gray-200 rounded p-3">
                          <div className="font-medium">{operation.operationName}</div>
                          <div className="text-sm text-gray-600">Исполнитель: {operation.executorName}</div>
                          <div className="text-sm text-gray-600">Дата: {formatDate(operation.executionDate)}</div>
                          <div className="text-sm text-gray-600">ОТК: {operation.qualityCheck ? 'Пройдено' : 'Не пройдено'}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value="Цех ортопедической обуви"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            readOnly
                          />
                        </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="font-medium text-gray-900">ORD-2025-0157</div>
                              <div className="text-sm text-gray-600 flex items-center space-x-1">
                                <User className="w-3 h-3" />
                                <span>Мария</span>
                              </div>
                              <div className="text-xs text-gray-500">Ортопедическая обувь | Травма</div>
...
export default FootwearInvoicesPage;

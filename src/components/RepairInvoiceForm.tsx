...
import { RepairInvoice, RepairInvoiceMaterial, RepairFitting } from '../types/invoices';
import { PersonalFile } from '../types/personalFile';
import { Order } from '../types';

interface RepairInvoiceFormProps {
  invoice?: RepairInvoice;
  onSave: (invoice: RepairInvoice) => void;
  onCancel: () => void;
  personalFiles?: PersonalFile[];
  orders?: Order[];
}

const RepairInvoiceForm: React.FC<RepairInvoiceFormProps> = ({
  invoice,
  onSave,
  onCancel,
  personalFiles = [],
  ...
  const [formData, setFormData] = useState<Partial<RepairInvoice>>({
    type: 'repair',
    status: 'draft',
    priority: 'normal',
    materials: [],
  ...
  const [newMaterial, setNewMaterial] = useState<Partial<RepairInvoiceMaterial>>({
    articleNumber: '',
    name: '',
    unit: '',
    quantity: 0,
  ...
  const [newFitting, setNewFitting] = useState<Partial<RepairFitting>>({
    fittingNumber: 1,
    callDate: '',
    appointmentDate: ''
  });
  ...
  const addMaterial = () => {
    if (newMaterial.articleNumber && newMaterial.name && newMaterial.quantity && newMaterial.price) {
      const material: RepairInvoiceMaterial = {
        id: `mat-${Date.now()}`,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const invoiceData: RepairInvoice = {
      id: formData.id || `inv-repair-${Date.now()}`,
      type: 'repair',
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900">
              {invoice ? 'Редактировать накладную на ремонт' : 'Создать накладную на ремонт'}
            </h2>
            <button
              onClick={onCancel}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {orders
                    .filter(order => order.productCategory === 'ремонт' || order.productType.toLowerCase().includes('ремонт'))
                    .map(order => (
              ...
              <div>
                ...
                >
                  ...
                  <option value="Ремонт протеза бедра">Ремонт протеза бедра</option>
                  <option value="Ремонт протеза голени">Ремонт протеза голени</option>
                  <option value="Ремонт протеза стопы">Ремонт протеза стопы</option>
                  <option value="Ремонт протеза кисти">Ремонт протеза кисти</option>
                  <option value="Ремонт ортеза">Ремонт ортеза</option>
                  <option value="Ремонт корсета">Ремонт корсета</option>
                  <option value="Ремонт бандажа">Ремонт бандажа</option>
                  <option value="Ремонт ортопедической обуви">Ремонт ортопедической обуви</option>
                  <option value="Профилактический ремонт">Профилактический ремонт</option>
            ...
            <div className="border rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Материалы для ремонта</h3>

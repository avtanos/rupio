...
export const adaptWorkOrderToOttobockOrder = (workOrder: WorkOrder): OttobockOrder | null => {
  ...
  // Значения по умолчанию для OttobockOrder
  const defaultProductType: OttobockProductType = 'Протез кисти';
  const defaultDiagnosis: OttobockDiagnosis = 'Ампутация правой кисти';
  const defaultSide: OttobockSide = 'Правый';
...
export const adaptOttobockOrderToWorkOrder = (ottobockOrder: OttobockOrder): WorkOrder => {
  return {
    ...
    movementStatus: 'В производстве',
    status: ottobockOrder.status === 'Срочный' ? 'в_производстве' : 'создан',
    orderType: 'Заказ на изготовление Оттобок',
    measurements: {},

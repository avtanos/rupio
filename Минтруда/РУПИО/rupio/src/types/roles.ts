export type UserRole = 'registration' | 'medical' | 'chief_doctor' | 'dispatcher' | 'workshop' | 'warehouse' | 'administration';
...
export const MODULE_PERMISSIONS = {
  ...
  personal_file: {
    ...
    chief_doctor: { view: true, create: false, edit: false, delete: false, print: true },
    dispatcher: { view: true, create: false, edit: false, delete: false, print: false },
...
export const ROLE_PERMISSIONS: Record<UserRole, string[]> = {
  ...
  chief_doctor: [
    'view_personal_files', 'print_personal_files',
...
export const ROLE_NAVIGATION: Record<UserRole, string[]> = {
  ...
  medical: ['process', 'medical', 'rehabilitation-directions', 'prosthesis-orders', 'footwear-orders', 'ottobock-orders', 'repair-orders', 'ready-poi-orders', 'reports', 'settings'],
  chief_doctor: ['process', 'approvals', 'medical-conclusions', 'reports', 'settings'],
  dispatcher: ['process', 'order-assignments', 'production-queue', 'reports', 'settings'],
  workshop: ['process', 'workshop', 'manufacturing-orders', 'prosthesis-orders', 'footwear-orders', 'ottobock-orders', 'repair-orders', 'ready-poi-orders', 'prosthesis-invoices', 'footwear-invoices', 'ottobock-invoices', 'repair-invoices', 'ready-poi-invoices', 'reports', 'settings'],
  warehouse: ['process', 'warehouse', 'warehouse-issuance', 'reports', 'settings'],
  administration: ['process', 'registration', 'medical', 'chief-doctor', 'dispatcher', 'workshop', 'warehouse', 'reports', 'prosthesis-orders', 'footwear-orders', 'ottobock-orders', 'repair-orders', 'ready-poi-orders', 'prosthesis-invoices', 'footwear-invoices', 'ottobock-invoices', 'repair-invoices', 'ready-poi-invoices', 'rehabilitation-directions', 'manufacturing-orders', 'warehouse-issuance', 'settings']
};
...
export const hasPermission = (
  ...
  if (!modulePermissions || !(userRole in modulePermissions)) {
    return false;
  }
  return modulePermissions[userRole as keyof typeof modulePermissions][permission];
};
...
export const getModulePermissions = (
  ...
  const modulePermissions = MODULE_PERMISSIONS[module];
  if (modulePermissions && userRole in modulePermissions) {
    return modulePermissions[userRole as keyof typeof modulePermissions];
  }
  
  // Возвращаем разрешения по умолчанию, если роль не найдена
  return {
    view: false,
    create: false,
    edit: false,

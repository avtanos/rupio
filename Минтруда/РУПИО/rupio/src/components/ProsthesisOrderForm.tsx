...
const ProsthesisOrderForm: React.FC<ProsthesisOrderFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!selectedPersonalFile) newErrors.personalFile = 'Личное дело пациента обязательно';
  ...
  const handleInputChange = (field: keyof ProsthesisOrderFormData, value: string | number | boolean | ProsthesisComponent[] | Fitting[] | ProsthesisDiagnosis | null) => {
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-6xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-2">
          ...
          <select
            value={selectedPersonalFile?.id || ''}
            onChange={(e) => handlePersonalFileChange(e.target.value)}
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
              errors.personalFile ? 'border-red-500' : 'border-gray-300'
            }`}
          >
          ...
          {errors.personalFile && (
            <p className="text-red-500 text-sm">{errors.personalFile}</p>
          )}
        </div>

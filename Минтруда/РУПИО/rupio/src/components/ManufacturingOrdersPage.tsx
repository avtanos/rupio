...
interface ManufacturingOrdersPageProps {
  ...
  onNewManufacturingOrder: (order: Omit<ManufacturingOrder, 'id' | 'orderNumber' | 'createdDate'>) => void;
  onUpdateManufacturingOrder: (id: string, updates: Partial<ManufacturingOrder>) => void;
  onDeleteManufacturingOrder: (id: string) => void;
  onNewTransferOrder: (transferOrder: Omit<TransferOrder, 'id' | 'transferNumber' | 'createdDate'>) => void;
}
...
const ManufacturingOrdersPage: React.FC<ManufacturingOrdersPageProps> = ({
  ...
  const handleUpdateMaterials = (materials: ConsumableMaterial[]) => {
    if (selectedOrder) {
      onUpdateManufacturingOrder(selectedOrder.id, { materials });
    }
  };

  const handleUpdateReadiness = (readiness: { stage: string; completionPercentage: number; notes?: string }) => {
    if (selectedOrder) {
      onUpdateManufacturingOrder(selectedOrder.id, { 
        readiness: {
          ...readiness,
  ...
  const handleSaveTransferOrder = (transferData: Omit<TransferOrder, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (selectedOrder) {
      onNewTransferOrder({
        ...transferData,

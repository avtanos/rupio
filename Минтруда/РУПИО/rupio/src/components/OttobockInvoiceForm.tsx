...
import { OttobockInvoice, OttobockInvoiceMaterial, OttobockFitting } from '../types/invoices';
import { PersonalFile } from '../types/personalFile';
import { Order } from '../types';

interface OttobockInvoiceFormProps {
  invoice?: OttobockInvoice;
  onSave: (invoice: OttobockInvoice) => void;
  onCancel: () => void;
  personalFiles?: PersonalFile[];
  orders?: Order[];
}

const OttobockInvoiceForm: React.FC<OttobockInvoiceFormProps> = ({
  invoice,
  onSave,
  onCancel,
  personalFiles = [],
  ...
  const [formData, setFormData] = useState<Partial<OttobockInvoice>>({
    type: 'ottobock',
    status: 'draft',
    priority: 'normal',
    materials: [],
  ...
  const [newMaterial, setNewMaterial] = useState<Partial<OttobockInvoiceMaterial>>({
    articleNumber: '',
    name: '',
    unit: '',
    quantity: 0,
  ...
  const [newFitting, setNewFitting] = useState<Partial<OttobockFitting>>({
    fittingNumber: 1,
    callDate: '',
    appointmentDate: ''
  });
  ...
  const addMaterial = () => {
    if (newMaterial.articleNumber && newMaterial.name && newMaterial.quantity && newMaterial.price) {
      const material: OttobockInvoiceMaterial = {
        id: `mat-${Date.now()}`,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const invoiceData: OttobockInvoice = {
      id: formData.id || `inv-ottobock-${Date.now()}`,
      type: 'ottobock',
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900">
              {invoice ? 'Редактировать накладную на Оттобок' : 'Создать накладную на Оттобок'}
            </h2>
            <button
              onClick={onCancel}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {orders
                    .filter(order => order.productCategory === 'оттобок' || order.productType.toLowerCase().includes('оттобок'))
                    .map(order => (
            ...
            <div className="border rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Материалы</h3>
...
export default OttobockInvoiceForm;

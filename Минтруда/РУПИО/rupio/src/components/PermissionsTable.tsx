...
const PermissionsTable: React.FC<PermissionsTableProps> = ({ currentRole }) => {
  ...
  const getRowClassName = (module: keyof typeof MODULE_PERMISSIONS) => {
    ...
    if (!modulePermissions || !(currentRole in modulePermissions)) {
      return '';
    }
    const hasAnyPermission = Object.values(modulePermissions[currentRole as keyof typeof modulePermissions]).some(Boolean);
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {modules.map((module, index) => (
              <tr key={module} className={getRowClassName(module)}>
                ...
                {roles.map(role => {
                  const modulePermissions = MODULE_PERMISSIONS[module];
                  if (!modulePermissions || !(role in modulePermissions)) {
                    return null;
                  }
                  const permissions = modulePermissions[role as keyof typeof modulePermissions];
                        ...
                        <div className="flex flex-col space-y-1">
                          {Object.entries(permissions).map(([permission, hasAccess]) => (
                            <div key={permission} className="flex items-center justify-center space-x-1">
                              {getPermissionIcon(Boolean(hasAccess))}

...
import { 
  ...
  OrthosisMaterial,
  OrthosisFitting
} from '../types/orthosisOrder';
import { PersonalFile } from '../types/personalFile';
import OrthosisMaterialsTable from './OrthosisMaterialsTable';
import OrthosisFittingsManager from './OrthosisFittingsManager';
import { X, Save, AlertCircle, Plus, Trash2 } from 'lucide-react';

interface OrthosisOrderFormProps {
  order?: OrthosisOrder;
  personalFile?: PersonalFile;
  personalFiles?: PersonalFile[]; // Добавляем список всех личных дел
  onSave: (order: OrthosisOrder) => void;
  onCancel: () => void;
  productTypes: OrthosisProductType[];
  diagnoses: OrthosisDiagnosis[];
  materialUnits: string[];
  defaultMaterials: OrthosisMaterial[];
}

const OrthosisOrderForm: React.FC<OrthosisOrderFormProps> = ({
  order,
  personalFile,
  personalFiles = [],
  onSave,
  ...
  const [formData, setFormData] = useState<OrthosisOrderFormData>({
    productType: 'Ортез коленного сустава',
    diagnosis: 'Артроз коленного сустава',
    side: 'Правый',
  ...
  const validateForm = () => {
    ...
    if (!formData.side) newErrors.side = 'Сторона обязательна';
  ...
  const handleInputChange = (field: keyof OrthosisOrderFormData, value: string | number | boolean | OrthosisMaterial[] | OrthosisFitting[]) => {
  ...
  const handleAddFitting = () => {
    ...
    const newFitting: OrthosisFitting = {
      id: `fit_${Date.now()}`,
      fittingNumber,
  ...
  const generateOrderNumber = () => {
    ...
    return `ОРТ-${currentYear}-${randomNumber.toString().padStart(3, '0')}`;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      const orderData: OrthosisOrder = {
        id: order?.id || `ORT${Date.now()}`,
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">
          {order ? 'Редактирование заказа на изготовление Оттобок' : 'Заказ на изготовление Оттобок'}
        </h2>
        <button
          onClick={onCancel}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('productType', e.target.value as OrthosisProductType)}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('diagnosis', e.target.value as OrthosisDiagnosis)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Сторона <span className="text-red-500">*</span>
              </label>
              <select
                value={formData.side}
                onChange={(e) => handleInputChange('side', e.target.value as OrthosisSide)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('orderType', e.target.value as OrthosisOrderType)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('status', e.target.value as OrthosisOrderStatus)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('serviceType', e.target.value as OrthosisServiceType)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
...
export default OrthosisOrderForm;
